...
/**
 * TOTP（RFC 6238）离线动态验证码工具（PC 端）。
 *
 * 与安卓端 A9.9.18 com.mimavault.util.TotpUtil 行为保持一致，保证两端对同一条目
 * 生成的验证码完全相同：
 ...
 *
 * 纯离线实现：不联网，密钥由调用方 AES-256-GCM 加密后存入本地库 totp_secret_enc 列。
 * HMAC 算法名形如 HmacSHA1 / HmacSHA256 / HmacSHA512，JDK 内置支持，无需第三方库。
 */
public final class TotpUtil {
    ...
    /** 按位数分组展示（6 位 → "123 456"，8 位 → "1234 5678"），便于肉眼核对 */
